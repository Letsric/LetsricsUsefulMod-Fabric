...
public class AutotextScreen extends Screen {
    ...
    public AutotextScreen() {
        super(Text.literal("Autotext Konfiguration"));
    }
    ...
    public void init() {
        for (int i = 0 ; i < AutoText.Autotexts.size() ; i++) {
            int i2 = i;
            addDrawableChild(new ButtonWidget(this.width / 2 + 100, i * 24 + 80, 98, 20, Text.literal("Entfernen"), action -> {
        ...
        addDrawableChild(new ButtonWidget(this.width - 100, 40, 25, 20, Text.literal("+"), action -> {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0 ; i < AutoText.Autotexts.size() ; i++) {
            drawTextWithShadow(matrices, this.textRenderer, Text.literal(LetsricsUsefulMod.autoTextArray.get(i).split(";")[1]), this.width / 2 - 100, i * 24 + 85, 0xFFFFFF);
        }
